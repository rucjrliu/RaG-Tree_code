...
public:
    ...
    // U-ALL(update all path) 追加入口：调用方传入追加到 Dataset 后得到的 id。
    ...
    // U-NO(update, no rebuild) 删除入口：只设置删除 bit，不改 R-tree/HNSW 结构。
...
private:
...
#ifdef RTREE_NSW_USE_HNSWLIB
            add_record_to_node_hnsw(*node, id);
#endif
        }
                       ...
                       less<pair<float, size_t>>> &topk) const
    {
        ...
        vector<pair<size_t, float>> recalled =
            stop_node.nsw_index->search_knn_filtered(query_vector, recall_k, &live_filter);
