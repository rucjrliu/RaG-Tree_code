...
PROGRAM="${RAGTREE_UPDATE_BRUTEFORCE_BIN:-./test_ragtree_update_bruteforce}"
PYTHON_BIN="${PYTHON_BIN:-${PYTHON:-python3}}"
CHECKER="checker_ragtree_update_bruteforce.py"
...
require_program() {
    [ -x "$PROGRAM" ] || die "$PROGRAM is not executable; compile test_ragtree_update_bruteforce.cpp first"
}
...
case "$mode" in
    ...
    1)
        ...
        mkdir -p ../output
        result_path="../output/result_ragtree_update_bruteforce_${dataset}_${update_name}_ef${ef_search}_${ada_flag}${opt_flag}.log"
        printf 'Running: %s %s\n' "$PROGRAM" "$*"
        "$PROGRAM" "$@" >"$result_path"
        printf 'Result: %s\n' "$result_path"
