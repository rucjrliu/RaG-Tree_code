...
struct NodeVectorIndex
{
    ...
    virtual void add_point(size_t id)
    {
        (void)id;
        throw runtime_error("NodeVectorIndex does not support online add_point");
    }
...
struct RTreeNode
{
    ...
    mutable vector<size_t> add_buffer; // U-ADA-V2: 尚未并入主 HNSW 的在线追加 id
    mutable float accumulated_delta_search_cost = 0.0f; // U-ADA-V2: 累计查询 delta HNSW 的代价
    unique_ptr<RTreeNode> lch; // 左孩子；当前实现是二叉划分，因此内部节点通常左右孩子都存在
    unique_ptr<RTreeNode> rch; // 右孩子
    unique_ptr<NodeVectorIndex> nsw_index; // 挂载在该结点上的主 HNSW 接口
    mutable unique_ptr<NodeVectorIndex> delta_nsw_index; // U-ADA-V2: add_buffer 上的 delta HNSW
...
public:
    ...
    float distance_to_id(const float *query_vector, size_t id) const
    {
        if (query_vector == nullptr)
        {
            throw invalid_argument("query vector must not be null");
        }
        return l2_func_(query_vector, dataset_->get_vector_i(id), l2_param_);
    }
...
public:
    ...
    void add_point(size_t id) override
    {
        if (!index_)
        {
            throw runtime_error("cannot add a point to an unbuilt hnswlib HNSW index");
        }

        if (index_->getCurrentElementCount() >= index_->getMaxElements())
        {
            const size_t current_capacity = index_->getMaxElements();
            const size_t next_capacity =
                max(index_->getCurrentElementCount() + 1,
                    current_capacity > 0 ? current_capacity * 2 : static_cast<size_t>(1));
            index_->resizeIndex(next_capacity);
        }
...
public:
    ...
    enum class RangeCompareMode
    {
        ...
        AVX2            // 6-8 个属性维度：一次 __m256 比较八个 float，不足维度用哨兵补齐
    };
    ...
    struct OptStopNode
    {
        ...
        float delta_graph_entry_cost = 0.0f;
        float delta_graph_search_cost = 0.0f;
        size_t delta_ef_search = 0;
    ...
    // U-ADA(update adaptive add buffer) 追加入口：调用方传入追加到 Dataset 后得到的 id。
    void add_record(size_t id)
    {
        ensure_delete_buffer_capacity(id);
        del_buffer_.reset(id);
        insert_record_into_tree(id);
    }

    // U-ADA 删除入口：只设置删除 bit，不改 R-tree/HNSW 结构。
...
#ifdef RTREE_NSW_USE_HNSWLIB
    // U-ADA-V2: add_record 之后由外部批量调用，给每个 HNSW 层结点的 add_buffer 构建 delta HNSW。
    void updateindex()
    {
        if (!root_)
        {
        ...
        build_delta_indexes_for_add_buffers(*root_);
    }
#endif

    bool is_record_deleted(size_t id) const
    {
        return is_deleted_id(id);
    }
    ...
    RTreeNode *build_tree()
    {
        ...
        // OPT planning needs child size weights immediately after build(),
        // matching the runtime statistics restored by load_tree_binary().
    ...
    void save_tree_binary(const char *path) const
    {
        ...
        if (root_)
        {
            flush_add_buffers_for_save(*root_);
        }
#endif
    ...
    void saveupdatedindex(const char *path) const
    {
        const string folder = normalize_folder_path(path);
        ensure_directory(folder);
        cerr << tree_binary_path(folder) << " ..." << endl;
        ...
        {
            save_node_indexes_binary(folder, *root_);
            save_delta_node_indexes_binary(folder, *root_);
        }
#endif
        save_update_state_binary(folder);
    }
    ...
    void loadupdatedindex(const char *path, bool rebuild_indexes = false)
    {
        if (rebuild_indexes)
        {
            throw invalid_argument("U-ADA loadupdatedindex cannot rebuild indexes while preserving add_buffer state");
        }
        ...
        {
            root_.reset();
            load_update_state_binary(folder);
            return;
        }
        ...
        load_node_indexes_binary_preserving_add_buffers(folder, *root_);
        load_delta_node_indexes_binary(folder, *root_);
#endif
    }
            ...
            for(const OptStopNode &stop_node_info : stop_nodes)
            {
                ...
                if (stop_node != nullptr)
                {
                    cerr << ' ' << stop_node->node_id
                         ...
                         << ",deltaef=" << stop_node_info.delta_ef_search
                         << ')';
                }
            }
            cerr << endl;
...
private:
    ...
    struct InsertPathEntry
    {
        ...
        bool expand_mbr = false;
    };

    struct InsertChildChoice
    {
        RTreeNode *child = nullptr;
        bool child_expand_mbr = false;
    };

    void initialize_update_buffers(size_t base_size)
    {
        if (base_size > kDeleteBufferBits)
        {
            throw overflow_error("initial Dataset size exceeds fixed delete bitset capacity");
        }
        del_buffer_.reset();
    }

    void ensure_delete_buffer_capacity(size_t id) const
    {
        if (id >= kDeleteBufferBits)
        {
            throw out_of_range("record id exceeds fixed delete bitset capacity");
        }
    }

    bool is_deleted_id(size_t id) const
    {
        return id < kDeleteBufferBits && del_buffer_.test(id);
    }

    void insert_record_into_tree(size_t id)
    {
        if (!root_)
        {
            throw runtime_error("add_record requires an existing R-tree; call build_tree/build first");
        }

        vector<InsertPathEntry> path;
        path.reserve(64);
        RTreeNode *leaf =
            locate_insert_path(*root_, id, !mbr_contains_data(root_->mbr, id), path);
        if (leaf == nullptr)
        {
            throw runtime_error("failed to locate an insertion path in R-tree");
        }

        for (const InsertPathEntry &entry : path)
        {
            RTreeNode *node = entry.node;
            if (node == nullptr)
            {
                continue;
            }
            node->covered_ids.push_back(id);
            if (entry.expand_mbr)
            {
                expand_node_mbr_for_data(*node, id);
            }
            node->add_buffer.push_back(id);
#ifdef RTREE_NSW_USE_HNSWLIB
            node->delta_nsw_index.reset();
#endif
            node->accumulated_delta_search_cost = 0.0f;
        }
        leaf->data_ids.push_back(id);

        n_ = max(n_, id + 1);
        refresh_tree_runtime_metrics();
        refresh_insert_path_statistics(path);
    }

    RTreeNode *locate_insert_path(RTreeNode &node,
                                  size_t id,
                                  bool expand_mbr,
                                  vector<InsertPathEntry> &path) const
    {
        path.push_back({&node, expand_mbr});
        if (node.is_leaf())
        {
            return &node;
        }

        const InsertChildChoice choice = choose_insert_child(node, id);
        if (choice.child == nullptr)
        {
            return &node;
        }
        return locate_insert_path(*choice.child, id, choice.child_expand_mbr, path);
    }

    InsertChildChoice choose_insert_child(RTreeNode &node, size_t id) const
    {
        RTreeNode *left = node.lch.get();
        RTreeNode *right = node.rch.get();
        if (left == nullptr)
        {
            return {right, right != nullptr && !mbr_contains_data(right->mbr, id)};
        }
        if (right == nullptr)
        {
            return {left, !mbr_contains_data(left->mbr, id)};
        }

        const bool left_contains = mbr_contains_data(left->mbr, id);
        const bool right_contains = mbr_contains_data(right->mbr, id);
        if (left_contains && !right_contains)
        {
            return {left, false};
        }
        if (right_contains && !left_contains)
        {
            return {right, false};
        }
        if (left->covered_ids.size() != right->covered_ids.size())
        {
            if (left->covered_ids.size() < right->covered_ids.size())
            {
                return {left, !left_contains};
            }
            return {right, !right_contains};
        }
        return {left, !left_contains};
    }

    bool mbr_contains_data(const BoundingBox &box, size_t id) const
    {
        if (box.low.size() != attr_dim_ || box.high.size() != attr_dim_)
        {
            return false;
        }
        return point_in_box(dataset_->get_attributes_i(id),
                            box.low.data(),
                            box.high.data(),
                            attr_dim_,
                            compare_mode_);
    ...
    void expand_node_mbr_for_data(RTreeNode &node, size_t id)
    {
        node.mbr.expand_point(attr_dim_,
                              [&](size_t axis)
                              {
...
#ifdef RTREE_NSW_USE_HNSWLIB
    void add_record_to_node_hnsw(RTreeNode &node, size_t id)
    {
        if (!node.hnsw_node)
        {
            return;
        ...
        if (!node.nsw_index)
        {
            throw runtime_error("add_record requires every HNSW-layer path node to have an existing HNSW index");
        }
        node.nsw_index->add_point(id);
    }
#endif

#ifdef RTREE_NSW_USE_HNSWLIB
    class LiveIdFilter final : public hnswlib::BaseFilterFunctor
    {
    public:
        explicit LiveIdFilter(const RTreeNSW *index)
            : index_(index)
        {
        }
        ...
        bool operator()(hnswlib::labeltype id) override
        {
            return index_ == nullptr ||
                   !index_->is_deleted_id(static_cast<size_t>(id));
        }

    private:
        const RTreeNSW *index_;
    };
#endif
    ...
    void refresh_insert_path_statistics(const vector<InsertPathEntry> &path) const
    {
        for (const InsertPathEntry &entry : path)
        {
            if (entry.node != nullptr && child_count(*entry.node) != 0)
            {
                calculate_node_statistics(*entry.node);
            }
        }
    }
    ...
    static constexpr uint64_t update_binary_magic()
    {
        return 0x52544E5357555031uLL; // "RTNSWUP1"
    }

    static constexpr uint64_t update_binary_version()
    {
        return 1;
    }

    static constexpr uint64_t update_binary_variant()
    {
        return 4; // U-ADA-V2
    }
    ...
    static string node_delta_hnsw_binary_path(const string &folder, size_t node_id)
    {
        ostringstream name;
        name << "treenode_" << node_id << "_delta_hnsw.bin";
        return join_path(folder, name.str());
    }

    static string update_state_binary_path(const string &folder)
    {
        return join_path(folder, "update_state.bin");
    }
    ...
    void write_update_state_header(ostream &out) const
    {
        write_pod(out, update_binary_magic());
        write_pod(out, update_binary_version());
        write_pod(out, update_binary_variant());
        write_size(out, n_);
        write_size(out, vector_dim_);
        write_size(out, attr_dim_);
    }

    void read_and_validate_update_state_header(istream &in)
    {
        const uint64_t magic = read_pod<uint64_t>(in);
        const uint64_t version = read_pod<uint64_t>(in);
        const uint64_t variant = read_pod<uint64_t>(in);
        if (magic != update_binary_magic() ||
            version != update_binary_version() ||
            variant != update_binary_variant())
        {
            throw runtime_error("RTreeNSW update state file has an unsupported format");
        }
        ...
        const size_t stored_attr_dim = read_size(in);
        if (stored_n != n_ ||
            stored_vector_dim != vector_dim_ ||
            stored_attr_dim != attr_dim_)
        {
            throw runtime_error("RTreeNSW update state file does not match the loaded Dataset");
        }
    }

    void write_delete_buffer_binary(ostream &out) const
    {
        write_size(out, del_buffer_.count());
        for (size_t id = 0; id < kDeleteBufferBits; ++id)
        {
            if (del_buffer_.test(id))
            {
                write_size(out, id);
            }
        }
    ...
    void read_delete_buffer_binary(istream &in)
    {
        del_buffer_.reset();
        const size_t deleted_count = read_size(in);
        for (size_t i = 0; i < deleted_count; ++i)
        {
            const size_t id = read_size(in);
            ensure_delete_buffer_capacity(id);
            del_buffer_.set(id);
        }
    }

    void validate_add_buffer_ids(const vector<size_t> &ids) const
    {
        for (size_t id : ids)
        {
            ensure_delete_buffer_capacity(id);
    ...
    void write_node_update_buffers_binary(ostream &out, const RTreeNode &node) const
    {
        write_size(out, node.node_id);
        write_size_vector(out, node.add_buffer);
        write_size(out, child_count(node));
        for_each_child(node,
                       [&](const RTreeNode &child, size_t)
                       {
                           write_node_update_buffers_binary(out, child);
                       });
    }

    void read_node_update_buffers_binary(istream &in, RTreeNode &node)
    {
        const size_t stored_node_id = read_size(in);
        if (stored_node_id != node.node_id)
        {
            throw runtime_error("RTreeNSW update state file node_id does not match tree.bin");
        }

        node.add_buffer = read_size_vector(in);
        validate_add_buffer_ids(node.add_buffer);
        node.accumulated_delta_search_cost = 0.0f;
#ifdef RTREE_NSW_USE_HNSWLIB
        node.delta_nsw_index.reset();
#endif

        const size_t stored_child_count = read_size(in);
        if (stored_child_count != child_count(node))
        {
            throw runtime_error("RTreeNSW update state file child_count does not match tree.bin");
        }
        for_each_child(node,
                       [&](RTreeNode &child, size_t)
                       {
                           read_node_update_buffers_binary(in, child);
                       });
    }

    void save_update_state_binary(const string &folder) const
    {
        ensure_directory(folder);
        ofstream out(update_state_binary_path(folder), ios::binary);
        if (!out)
        {
            throw runtime_error("failed to open RTreeNSW update state file for writing");
        }

        write_update_state_header(out);
        write_delete_buffer_binary(out);
        write_bool(out, root_ != nullptr);
        if (root_)
        {
            write_node_update_buffers_binary(out, *root_);
        }
        out.close();
        if (!out)
        {
            throw runtime_error("failed to close RTreeNSW update state file after writing");
        }
    }

    void load_update_state_binary(const string &folder)
    {
        ifstream in(update_state_binary_path(folder), ios::binary);
        if (!in)
        {
            throw runtime_error("failed to open RTreeNSW update state file for reading");
        }

        read_and_validate_update_state_header(in);
        read_delete_buffer_binary(in);
        const bool state_has_root = read_bool(in);
        if (state_has_root != static_cast<bool>(root_))
        {
            throw runtime_error("RTreeNSW update state root flag does not match tree.bin");
        }
        if (root_)
        {
            read_node_update_buffers_binary(in, *root_);
        }
    }
    ...
    size_t delta_max_neighbors_for_node(const RTreeNode &node) const
    {
        return max(static_cast<size_t>(1),
                   opt_effective_max_neighbors(node) / static_cast<size_t>(2));
    }

    size_t delta_ef_construction_for_node(const RTreeNode &node) const
    {
        return delta_max_neighbors_for_node(node) * static_cast<size_t>(4);
    }

    unique_ptr<NodeVectorIndex> build_delta_hnsw_index_for_node(const RTreeNode &node) const
    {
        if (!node.hnsw_node || node.add_buffer.empty())
        {
            return nullptr;
        }

        const size_t delta_max_neighbors = delta_max_neighbors_for_node(node);
        const size_t delta_ef_construction = delta_ef_construction_for_node(node);
        cerr << "Building RTree node" << node.node_id << print_mbr(node)
             << " DeltaHNSW(max_neighbor=" << delta_max_neighbors
             << ", ef_construction=" << delta_ef_construction
             << ") on " << node.add_buffer.size() << " data tuples ..." << endl;
        auto index = make_unique<HnswlibHNSWIndex<DatasetT>>(
            *dataset_,
            vector_dim_,
            delta_max_neighbors,
            delta_ef_construction,
            hnsw_ef_search_setting_,
            hnsw_random_seed_);
        index->build(node.add_buffer);
        return index;
    }

    void build_delta_indexes_for_add_buffers(RTreeNode &node)
    {
        node.delta_nsw_index.reset();
        node.accumulated_delta_search_cost = 0.0f;
        if (node.hnsw_node && !node.add_buffer.empty())
        {
            node.delta_nsw_index = build_delta_hnsw_index_for_node(node);
            if (!node.delta_nsw_index)
            {
                throw runtime_error("failed to build U-ADA-V2 delta HNSW index");
            }
        }
        for_each_child(node,
                       [&](RTreeNode &child, size_t)
                       {
                           build_delta_indexes_for_add_buffers(child);
                       });
    }
    ...
    void save_delta_node_indexes_binary(const string &folder, const RTreeNode &node) const
    {
        if (node.hnsw_node && !node.add_buffer.empty())
        {
            if (!node.delta_nsw_index)
            {
                throw runtime_error("cannot save U-ADA-V2 update index: call updateindex() before saveupdatedindex()");
            }
            node.delta_nsw_index->save_index_binary(
                node_delta_hnsw_binary_path(folder, node.node_id));
        }

        for_each_child(node,
                       [&](const RTreeNode &child, size_t)
                       {
                           save_delta_node_indexes_binary(folder, child);
                       });
    }
    ...
    void load_delta_node_indexes_binary(const string &folder, RTreeNode &node)
    {
        load_delta_node_index_binary(folder, node);
        for_each_child(node,
                       [&](RTreeNode &child, size_t)
                       {
                           load_delta_node_indexes_binary(folder, child);
                       });
    }

    vector<size_t> indexed_ids_excluding_add_buffer(const RTreeNode &node) const
    {
        if (node.add_buffer.empty())
        {
            return node.covered_ids;
        }

        vector<size_t> buffer_ids = node.add_buffer;
        sort(buffer_ids.begin(), buffer_ids.end());
        if (adjacent_find(buffer_ids.begin(), buffer_ids.end()) != buffer_ids.end())
        {
            throw runtime_error("RTreeNSW update state file contains duplicated node add_buffer ids");
        }

        vector<size_t> indexed_ids;
        indexed_ids.reserve(node.covered_ids.size());
        size_t excluded_count = 0;
        for (size_t id : node.covered_ids)
        {
            if (binary_search(buffer_ids.begin(), buffer_ids.end(), id))
            {
                ++excluded_count;
                continue;
            }
        ...
        if (excluded_count != buffer_ids.size())
        {
            throw runtime_error("RTreeNSW update state file add_buffer is not covered by tree.bin");
        }
        return indexed_ids;
    }

    void load_node_indexes_binary_preserving_add_buffers(const string &folder, RTreeNode &node)
    {
        load_node_index_binary_preserving_add_buffer(folder, node);
        for_each_child(node,
                       [&](RTreeNode &child, size_t)
                       {
                           load_node_indexes_binary_preserving_add_buffers(folder, child);
                       });
    }

    string print_mbr(const RTreeNode &node) const
    {
    ...
    void load_node_index_binary_preserving_add_buffer(const string &folder, RTreeNode &node)
    {
        node.nsw_index.reset();
        if (!node.hnsw_node || node.covered_ids.empty())
        {
        ...
        const vector<size_t> indexed_ids = indexed_ids_excluding_add_buffer(node);
        ...
        auto index = make_unique<HnswlibHNSWIndex<DatasetT>>(
            *dataset_,
            ...
            hnsw_ef_search_setting_,
            hnsw_random_seed_);
        index->load_index_binary(indexed_ids, node_hnsw_binary_path(folder, node.node_id));
        node.nsw_index = std::move(index);
        cerr << node_hnsw_binary_path(folder, node.node_id)
             << " (indexed_size=" << indexed_ids.size()
             << ", covered_size=" << node.covered_ids.size()
             << ", add_buffer=" << node.add_buffer.size() << ")"
             << print_mbr(node)
             << ' ' << print_node_weights(node)
             << " loaded" << endl;
    }

    void load_delta_node_index_binary(const string &folder, RTreeNode &node)
    {
        node.delta_nsw_index.reset();
        node.accumulated_delta_search_cost = 0.0f;
        if (!node.hnsw_node || node.add_buffer.empty())
        ...
        auto index = make_unique<HnswlibHNSWIndex<DatasetT>>(
            *dataset_,
            ...
            hnsw_ef_search_setting_,
            hnsw_random_seed_);
        index->load_index_binary(node.add_buffer,
                                 node_delta_hnsw_binary_path(folder, node.node_id));
        node.delta_nsw_index = std::move(index);
        cerr << node_delta_hnsw_binary_path(folder, node.node_id)
             << " (delta_size=" << node.add_buffer.size()
             << ", M=" << delta_max_neighbors
             << ", efc=" << delta_ef_construction << ")"
             << print_mbr(node)
             << " loaded" << endl;
    }
    ...
    void apply_hnsw_ef_search(RTreeNode &node, size_t ef_search)
    {
        ...
        if (node.delta_nsw_index)
        {
            node.delta_nsw_index->set_ef_search(ef_search);
        }
        for_each_child(node,
                       [&](RTreeNode &child, size_t)
                       {
...
#endif
#if defined(__AVX2__)
        // dim=6/7 时无效 lane 用 -inf/+inf padding，使其永远不会导致失败。
    ...
    RangeQueryState make_range_query_state(const QueryT &query) const
    {
        ...
        if (attr_dim_ > 8)
        {
            throw invalid_argument("rtree_hnsw_ori_v3 expects attribute dimension <= 8");
        }
        ...
        {
            // 2-5 维的前 4 维查询边界只需加载一次，后续对每个 MBR/point 复用。
    ...
    {
        // 根据属性维度选择最窄但足够的比较路径。
        // 这样 1/2/4/8 维都可以走简单固定宽度分支，避免每条数据循环判断维度。
        if (attr_dim_ == 1)
        {
        ...
        {
            compare_mode_ = RangeCompareMode::AVX2;
        }
        else
        {
            throw invalid_argument("rtree_hnsw_ori_v3 expects attribute dimension <= 8");
    ...
    static bool point_in_box(const float *values,
                             const float *box_low,
                             const float *box_high,
                             size_t dim,
                             RangeCompareMode mode)
    {
        switch (mode)
        {
        case RangeCompareMode::Scalar1:
            return !(values[0] < box_low[0] || values[0] > box_high[0]);
        case RangeCompareMode::SSE2:
            return point_in_box_sse2_4(values, box_low, box_high, dim);
        case RangeCompareMode::SSE2PlusScalar:
            return point_in_box_sse2_4(values, box_low, box_high, dim) &&
                   !(values[4] < box_low[4] || values[4] > box_high[4]);
        case RangeCompareMode::AVX2:
            return point_in_box_avx2_8(values, box_low, box_high, dim);
        }
        return false;
    }
    ...
    {
        // dim=4/5 可直接取前 4 维；dim=2/3 需要手动补无效 lane。
        switch (dim)
        {
        case 2:
    ...
    static bool point_in_box_sse2_4(const float *values,
                                    const float *box_low,
                                    const float *box_high,
                                    size_t dim)
    {
#if defined(__SSE2__)
        const __m128 v = load4_padded_sse2(values, dim, 0.0f);
        const __m128 bl = load4_padded_sse2(box_low, dim, -numeric_limits<float>::infinity());
        const __m128 bh = load4_padded_sse2(box_high, dim, numeric_limits<float>::infinity());
        const __m128 fail_low = _mm_cmplt_ps(v, bl);
        const __m128 fail_high = _mm_cmpgt_ps(v, bh);
        return _mm_movemask_ps(_mm_or_ps(fail_low, fail_high)) == 0;
#else
        for (size_t j = 0; j < dim && j < 4; ++j)
        {
            if (values[j] < box_low[j] || values[j] > box_high[j])
            {
                return false;
            }
        }
    ...
    static bool point_in_box_avx2_8(const float *values,
                                    const float *box_low,
                                    const float *box_high,
                                    size_t dim)
    {
#if defined(__AVX2__)
        const __m256 v = load8_padded_avx2(values, dim, 0.0f);
        const __m256 bl = load8_padded_avx2(box_low, dim, -numeric_limits<float>::infinity());
        const __m256 bh = load8_padded_avx2(box_high, dim, numeric_limits<float>::infinity());
        const __m256 fail_low = _mm256_cmp_ps(v, bl, _CMP_LT_OQ);
        const __m256 fail_high = _mm256_cmp_ps(v, bh, _CMP_GT_OQ);
        return _mm256_movemask_ps(_mm256_or_ps(fail_low, fail_high)) == 0;
#else
    ...
    size_t opt_effective_ef_construction(const RTreeNode &node) const
    {
#ifdef RTREE_NSW_USE_HNSWLIB
        if (node.hnsw_ef_construction != 0)
        {
            return node.hnsw_ef_construction;
        }
    ...
    float add_buffer_absorb_cost_threshold(const RTreeNode &node) const
    {
        const size_t buffer_size = node.add_buffer.size();
        if (buffer_size == 0)
        {
            return 0.0f;
        }

        const float max_neighbors =
            static_cast<float>(opt_effective_max_neighbors(node));
        const float ef_construction =
            static_cast<float>(opt_effective_ef_construction(node));
        const float h = static_cast<float>(node.hnsw_height);
        const float per_point_cost =
            max_neighbors * max_neighbors * h +
            2.0f * max_neighbors * ef_construction;
        return static_cast<float>(buffer_size) * per_point_cost;
    }

    size_t delta_ef_search_for_plan(const RTreeNode &node,
                                    float planned_ef_search) const
    {
        const size_t buffer_size = node.add_buffer.size();
        if (buffer_size == 0)
        {
        ...
        const size_t indexed_size =
            node.covered_ids.size() >= buffer_size
                ? node.covered_ids.size() - buffer_size
                : 0;
        if (indexed_size == 0)
        {
        ...
        const float ef_search =
            isfinite(planned_ef_search) && planned_ef_search > 0.0f
                ? planned_ef_search
                : 0.0f;
        const double raw_delta_ef =
            static_cast<double>(ef_search) *
            static_cast<double>(buffer_size) /
            static_cast<double>(indexed_size);
        if (!isfinite(raw_delta_ef) || raw_delta_ef <= 0.0)
        {
            return 0;
        }
        const size_t delta_ef =
            static_cast<size_t>(round(raw_delta_ef));
        return min(buffer_size, delta_ef);
    }

    float delta_hnsw_height_for_plan(const RTreeNode &node) const
    {
        if (node.add_buffer.empty())
        {
            return 0.0f;
        ...
        return static_cast<float>(
            log2(static_cast<double>(node.add_buffer.size()) + 1.0));
    }

    float delta_graph_entry_cost_for_plan(const RTreeNode &node,
                                          size_t delta_ef_search) const
    {
        if (delta_ef_search == 0 || node.add_buffer.empty())
        {
            return 0.0f;
        }
        const float delta_max_neighbors =
            static_cast<float>(delta_max_neighbors_for_node(node));
        return delta_max_neighbors *
               delta_max_neighbors *
               delta_hnsw_height_for_plan(node);
    }

    float delta_graph_search_cost_for_plan(const RTreeNode &node,
                                           size_t delta_ef_search,
                                           float probability) const
    {
        ...
        const float delta_max_neighbors =
            static_cast<float>(delta_max_neighbors_for_node(node));
        return opt_graph_search_cost_from_probability(
            static_cast<float>(node.add_buffer.size()),
            delta_max_neighbors,
            static_cast<float>(delta_ef_search),
            probability);
    }
                                       ...
                                       numeric_limits<float>::quiet_NaN()) const
    {
        ...
        cost.graph_search_cost =
            opt_graph_search_cost_from_probability(node_size_f,
                                                   ...
                                                   search_width,
                                                   probability);
        cost.delta_ef_search =
            delta_ef_search_for_plan(node, context.ef_search);
        cost.delta_graph_entry_cost =
            delta_graph_entry_cost_for_plan(node, cost.delta_ef_search);
        cost.delta_graph_search_cost =
            delta_graph_search_cost_for_plan(node,
                                             cost.delta_ef_search,
                                             probability);
        const float merge_cost = 0.0f;
        cost.total_cost = cost.graph_entry_cost +
                          cost.graph_search_cost +
                          cost.delta_graph_entry_cost +
                          cost.delta_graph_search_cost +
                          merge_cost;
...
#ifdef RTREE_NSW_USE_HNSWLIB
    void absorb_node_add_buffer_into_main_hnsw(const RTreeNode &node) const
    {
        if (node.add_buffer.empty())
        {
            return;
        ...
        if (!node.nsw_index)
        {
            throw runtime_error("U-ADA-V2 add_buffer absorb requires an existing main HNSW index");
        }

        for (size_t id : node.add_buffer)
        {
            node.nsw_index->add_point(id);
        }
        node.add_buffer.clear();
    ...
    void flush_add_buffers_for_save(const RTreeNode &node) const
    {
        absorb_node_add_buffer_into_main_hnsw(node);
        for_each_child(node,
                       [&](const RTreeNode &child, size_t)
                       {
                           flush_add_buffers_for_save(child);
                       });
    }

    void query_delta_hnsw_into_topk(
        const RTreeNode &node,
        const float *query_vector,
        const RangeQueryState &state,
        bool node_fully_contained,
        size_t delta_ef_search,
        float planned_delta_query_cost,
        size_t k,
        priority_queue<pair<float, size_t>,
                       vector<pair<float, size_t>>,
                       less<pair<float, size_t>>> &topk) const
    {
        if (query_vector == nullptr ||
            k == 0 ||
            delta_ef_search == 0 ||
            node.add_buffer.empty())
        {
            return;
        }
        if (!node.delta_nsw_index)
        {
            throw runtime_error("U-ADA-V2 delta HNSW is missing; call updateindex() before querying add_buffer");
        }

        const size_t realized_delta_ef = min(delta_ef_search,
                                            node.add_buffer.size());
        if (realized_delta_ef == 0)
        {
            return;
        }

        const float delta_query_cost =
            isfinite(planned_delta_query_cost) && planned_delta_query_cost > 0.0f
                ? planned_delta_query_cost
                : 0.0f;
        if (node.accumulated_delta_search_cost + delta_query_cost >
            add_buffer_absorb_cost_threshold(node))
        {
            absorb_node_add_buffer_into_main_hnsw(node);
            return;
        }

        node.delta_nsw_index->set_ef_search(realized_delta_ef);
        LiveIdFilter live_filter(this);
        vector<pair<size_t, float>> recalled =
            node.delta_nsw_index->search_knn_filtered(query_vector,
                                                      realized_delta_ef,
                                                      &live_filter);
        post_filter_recall_into_topk(recalled, node_fully_contained, state, k, topk);

        node.accumulated_delta_search_cost += delta_query_cost;
    }
    ...
    void query_hnsw_recall_into_topk(
        ...
        priority_queue<pair<float, size_t>,
                       vector<pair<float, size_t>>,
                       less<pair<float, size_t>>> &topk,
        size_t delta_ef_search = 0,
        float planned_delta_query_cost = 0.0f) const
    {
        if (k == 0 || stop_node.covered_ids.empty())
        {
            return;
        }

        query_delta_hnsw_into_topk(stop_node,
                                   query_vector,
                                   state,
                                   node_fully_contained,
                                   delta_ef_search,
        ...
        if (stop_node.nsw_index)
        {
            ...
            const size_t indexed_node_size =
                stop_node.covered_ids.size() >= stop_node.add_buffer.size()
                    ? stop_node.covered_ids.size() - stop_node.add_buffer.size()
                    : 0;
            const size_t recall_k = min(realized_ef_search, indexed_node_size);
            if (recall_k != 0)
            {
                stop_node.nsw_index->set_ef_search(realized_ef_search);
                LiveIdFilter live_filter(this);
                vector<pair<size_t, float>> recalled =
                    stop_node.nsw_index->search_knn_filtered(query_vector,
                                                             recall_k,
                                                             &live_filter);
                post_filter_recall_into_topk(recalled, node_fully_contained, state, k, topk);
            }
        ...
        const char *debug_label) const
    {
        ...
        if (!stop_node.nsw_index && !stop_node.delta_nsw_index)
        {
            if (!query_options.exact_fallback_without_index)
            {
                throw runtime_error("RFANNS stop node has no main or delta hnswlib HNSW index; call build()/build_node_indexes()/updateindex()");
            }

            // 仅调试用兜底路径：不是 ANN，而是在 stop node 覆盖的数据上做精确 top-k。
            // 正式 RFANNS 默认关闭该兜底，因为所有非空 HNSW/query 层节点都应有 hnswlib HNSW。
            using HeapItem = pair<float, size_t>;
            priority_queue<HeapItem, vector<HeapItem>, less<HeapItem>> topk;
            const vector<pair<size_t, float>> exact_result =
        ...
        const size_t delta_ef_search =
            delta_ef_search_for_plan(stop_node,
                                     static_cast<float>(ef_search));
        const float delta_probability =
            node_fully_contained
                ? 1.0f
                : opt_clamp_probability(static_cast<float>(
                      opt_ce_range_probability(stop_node, state)));
        const float planned_delta_query_cost =
            delta_graph_entry_cost_for_plan(stop_node, delta_ef_search) +
            delta_graph_search_cost_for_plan(stop_node,
                                             delta_ef_search,
                                             delta_probability);
                                      ...
                                      const OptQueryContext &context) const
    {
        ...
        if (!node.nsw_index && !node.delta_nsw_index)
        {
            throw runtime_error("actual OPT RFANNS stop node has no main or delta hnswlib HNSW index; call build()/build_node_indexes()/updateindex()");
        }
        ...
        const float delta_probability =
            node_fully_contained
                ...
                : opt_clamp_probability(static_cast<float>(
                      opt_ce_range_probability(node, context.state)));
        const float planned_delta_query_cost =
            delta_graph_entry_cost_for_plan(node, delta_ef_search) +
            delta_graph_search_cost_for_plan(node,
                                             delta_ef_search,
                                             delta_probability);
                                     ...
                                     vector<OptStopNode> &stop_nodes) const
    {
        ...
        const float delta_probability =
            node_fully_contained
                ...
                : opt_clamp_probability(static_cast<float>(
                      opt_ce_range_probability(node, context.state)));
        const float delta_graph_entry_cost =
            delta_graph_entry_cost_for_plan(node, delta_ef_search);
        const float delta_graph_search_cost =
            delta_graph_search_cost_for_plan(node,
                                             delta_ef_search,
                                             delta_probability);
        stop_nodes.push_back({&node,
                              ...
                              0.0,
                              0.0,
                              delta_graph_entry_cost,
                              delta_graph_search_cost,
                              delta_ef_search,
                              0.0,
                              0.0});
        return cost;
                       ...
                       less<pair<float, size_t>>> &topk) const
    {
        ...
        if (!stop_node.nsw_index && !stop_node.delta_nsw_index)
        {
            if (!query_options.exact_fallback_without_index)
            {
                throw runtime_error("OPT RFANNS stop node has no main or delta hnswlib HNSW index; call build()/build_node_indexes()/updateindex()");
            }
        ...
        query_hnsw_recall_into_topk(stop_node,
                                    ...
                                    k,
                                    topk,
                                    stop_node_info.delta_ef_search,
                                    stop_node_info.delta_graph_entry_cost +
                                        stop_node_info.delta_graph_search_cost);
#ifdef DP_DEBUG
        const clock_t hnsw_end = clock();
        const double hnsw_time_ms =
            (hnsw_end - hnsw_start) * 1000.0 / CLOCKS_PER_SEC;
        const double plan_cost =
            stop_node_info.graph_entry_cost +
            stop_node_info.graph_search_cost +
            stop_node_info.delta_graph_entry_cost +
            stop_node_info.delta_graph_search_cost;
        ...
        if (!stop_node_info.actual_plan)
        {
            cerr << " graph_entry_cost=" << stop_node_info.graph_entry_cost
                 ...
                 << " delta_graph_entry_cost=" << stop_node_info.delta_graph_entry_cost
                 << " delta_graph_search_cost=" << stop_node_info.delta_graph_search_cost
                 << " delta_ef_search=" << stop_node_info.delta_ef_search
                                                   ...
                                                   bool sort_result = true) const
    {
        ...
        for (size_t id : ids)
        {
            if (is_deleted_id(id) || !data_matches_query(id, state))
            {
                continue;
            }
    ...
    static constexpr size_t kDeleteBufferBits = 16777216;
    bitset<kDeleteBufferBits> del_buffer_; // U-NO 删除 bitset：true 表示屏蔽该全局 id
